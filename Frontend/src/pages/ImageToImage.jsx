...
import { Image as ImageIcon, UploadCloud, Download, Share2, FolderPlus, Loader2, ArrowLeft, X } from 'lucide-react';
...
const ImageToImage = () => {
    const {
        ...
        handleImageUpload, handleGenerate, downloadImage, clearImages, removeImage
    } = useStudioTool('image-to-image');
        ...
        <div className="max-w-[1400px] mx-auto py-8 sm:py-12 px-4 sm:px-6 animate-[fadeIn_0.5s_ease-out]">
            <div className="flex items-center gap-4 mb-8">
                ...
                <h1 className="text-3xl sm:text-4xl font-extrabold flex items-center gap-3 text-[var(--text-primary)] tracking-tight">
                    <ImageIcon className="text-orange-500 w-8 h-8 drop-shadow-[0_0_15px_rgba(249,115,22,0.5)]"/> Image to Image
                </h1>
            </div>
                    ...
                    <div className="glass p-6 sm:p-8 rounded-3xl shadow-2xl border-[var(--border-subtle)] relative overflow-hidden">
                        ...
                        <div className="mb-8">
                            ...
                            <div 
                                ...
                                className="border-2 border-dashed border-[var(--border-subtle)] rounded-2xl p-6 flex flex-col items-center justify-center relative hover:bg-[var(--bg-card-hover)] hover:border-orange-500/50 transition-all cursor-pointer min-h-[160px] group bg-[var(--bg-card)]"
                            >
                                <input id="image-upload-input" type="file" accept="image/*" multiple onChange={handleImageUpload} onClick={(e) => { e.target.value = null; }} className="hidden" />
                                {sourceImagePreviews.length > 0 ? (
                                    <div className="flex gap-4 flex-wrap justify-center relative z-20">
                                        {sourceImagePreviews.map((preview, idx) => (
                                            <div key={idx} className="relative group/img">
                                                <img src={preview} alt="Preview" className="h-32 object-contain rounded-xl shadow-lg border border-[var(--border-subtle)]" />
                                                <button 
                                                    onClick={(e) => { e.stopPropagation(); removeImage(idx); }}
                                                    className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover/img:opacity-100 transition-opacity shadow-md hover:bg-red-600 z-40 cursor-pointer"
                                                    title="Remove image"
                                                >
                                                    <X className="w-4 h-4" />
                                                </button>
                                    ...
                                    <div className="text-center text-[var(--text-secondary)] group-hover:text-orange-500 transition-colors">
                                        ...
                                        <p className="text-sm font-medium">Click or drag image(s) here</p>
                                    </div>
                                )}
                            </div>
                        </div>
